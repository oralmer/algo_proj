...
EllipticCurveHasher::EllipticCurveHasher(nlohmann::json params) : ThresholdHasher(params) {
    ...
    m_cache.curr_in = std::vector<uint8_t>(4 + LABEL.length() + P_str.size());
    ...
    PackShort(m_cache.curr_in, length, 2 + LABEL.size() + P_str.size());
...
EllipticCurveHasher::DeriveKey(const std::vector<uint8_t> &password, const std::vector<uint8_t> &hash) const {
    HMACpp(hash, password, m_cache.seed.data(), m_cache.hmac);
    ...
    for ((*i) = 1; (*i) <= m_cache.num_of_iters; ++(*i)) {
        HMACpp(m_cache.seed, m_cache.curr_in, m_cache.result_piece.data(), m_cache.hmac);
