...
int main(int argc, char *argv[]) {
    if (argc < 7 || strcmp(argv[1], "-h") == 0) {
        std::cerr << "Usage: " << argv[0] << " [START] [END] [HASH TYPE] [PASSWORD TYPE] [PASS LEN] [HASH]" << std::endl;
    }
    ...
    PasswordGenerator password_generator = PasswordGenerator(password_type, password_len);
    Hasher hasher = Hasher(hash_type);

    for (int i = start; i < password_generator.Count() && i < end; i++) {
